...
class clase_pregunta():
    ...
    def transformar_tabla(nuevo_df):
        ...
        for uno in bus:
            ...
            if uno in nuevo_df[nombres_iniciales[2]].values: #nombres iniciales [2] es una referencia a la columna unnamed 2, pero ésta aveces cambia de nombre porque se borra cuando hay una mala lectura de la tabla
                
                comprobador.append(1)
                c = 0
                index = 0
                for fila in nuevo_df[nombres_iniciales[2]].values:
                    if uno in str(fila):
                        index = c
                        break
                    c += 1
